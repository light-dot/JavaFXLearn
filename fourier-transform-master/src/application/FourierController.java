...
/*
 * The controller associated to the only view of our application.
 * The application logic is implemented here.
 * It handles the button for opening an image and perform all the operation related to the Fourier transformation and antitransformation.
 * 
 * 与我们应用程序唯一视图相关的控制器
 * 应用程序逻辑在这里实现
 * 它处理打开图像的按钮，并执行与傅里叶变换和反傅里叶变换相关的所有操作
 * */
public class FourierController {
	...
	protected void loadImage() {
		//show the open dialog window 展示打开的会话窗口
		File file = this.fileChooser.showOpenDialog(this.stage);
		if (file != null) {
			//read the image in gray scale 灰度扫描读取图像
	...
	/*
	 * The action triggered by pushing the button for apply the dft to the loaded image
	 * */
	@FXML
	protected void transformImage() {
		//optimize the dimension of the loaded iamge
	...
	/*
	 *The action triggered by pushing the button for apply the inverse dft to the loaded image
	 * 
	 */
	@FXML
	...
	private Mat optimizeImageDim(Mat image) {
		...
		int addPiexlRows = Core.getOptimalDFTSize(image.rows());
		//get the optimal cols size for dft
		int addPiexlCols = Core.getOptimalDFTSize(image.cols());
		//apply the optimal cols and rows size to the image
		Core.copyMakeBorder(image,padded,0,addPiexlRows-image.rows(),0,addPiexlCols-image.cols(),
				Core.BORDER_CONSTANT,Scalar.all(0));
		
		return padded;
	}
	/*
	 * Optimize the magnitude(大小) of the complex image obtained from the DFT,to improve its visualization
	 * 
	 * @param complexImage
	 * 			the complex image obtained from the DTT
	...
	private Mat createOptimizedMagnitude(Mat complexImage) {
		...
		Core.log(mag,mag);
		//optionally reorder the 4 quadrants(象限) of the magnitude image
		this.shiftDFT(mag);
		...
		//Imgcodecs.inwrite("../magnitude.png",mag);
